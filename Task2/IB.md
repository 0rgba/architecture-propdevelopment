...
### Управление доступом
Требуется более строго контролировать доступ и ограничивать полномочия.

Самые важные подпункты:
...
Они позволят минимизировать риски неправомерного использования учётной записи
...
Самые важные подпункты:
* Разработана ли политика классификации данных? - необходимо для разграничения доступов
* Используется ли шифрование на уровне базы данных и при передаче данных по сети? - данные могут передаваться через небезопасные окружения, необходима защита

### Защита инфраструктуры
Для усложнения попыток несанкционированного доступа и быстрой реакции на них - использовать сегментацию сети, межсетевые экраны и системы обнаружения вторжений
...
Для своевременной реакции на инциденты требуются системы мониторинга, механизмы оповещений и описанные протоколы реагирования.
...
| 1   | Используется ли многофакторная аутентификация (MFA) для доступа к критическим системам и сервисам?                                          | нет    | в описании нет информации об этом   |
...
| 6   | Ведётся ли журналирование всех операций доступа к системам и данным? Кто имеет доступ к журналам?                                           | нет    | из описания это понятно не до конца |
...
| 1   | Используется ли межсетевой экран для ограничения входящего и исходящего трафика?                                                            | да     | для внешних пользователе            |
